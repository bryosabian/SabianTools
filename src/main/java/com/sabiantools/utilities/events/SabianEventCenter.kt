 ...
 * The event center
 */
class SabianEventCenter {

    /**
     * The events
     */
    private var events: LinkedHashMap<String, SabianEvent> = linkedMapOf()
     ...
     * @param observer The observer
     */
    inline fun <reified T : SabianEvent> subscribeTo(channel: String, observer: SabianEventObserver) {
     ...
     */
    @Throws(IllegalArgumentException::class)
    fun trigger(channel: String, payload: SabianEventPayload) {
        val event = getEvent(channel)
            ?: throw IllegalArgumentException("No event subscription found for that channel")
     ...
     */
    @Throws(IllegalArgumentException::class)
    fun triggerAndReturn(channel: String, payload: SabianEventPayload): SabianEventPayload {
